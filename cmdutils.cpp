                         ...
                         const char *arg)
{
    ...
    l->groups = static_cast<OptionGroup *>(grow_array(l->groups, sizeof(*(l->groups)),
            &(l->nb_groups),(l->nb_groups) + 1));
                    ...
                    const char *key, const char *val)
{
    ...
    g->opts = static_cast<Option *>(grow_array(g->opts, sizeof(*(g->opts)),
                                               &(g->nb_opts), (g->nb_opts) + 1));
                               ...
                               const OptionGroupDef *groups, int nb_groups)
{
    ...
    octx->groups    = static_cast<OptionGroupList *>(av_mallocz_array(octx->nb_groups, sizeof(*octx->groups)));
    if (!octx->groups)
        exit_program(1);
...
static void expand_filename_template(AVBPrint *bp, const char *filename_template,
                                     struct tm *tm)
{
    int c;

    while ((c = *(filename_template++))) {
        if (c == '%') {
            if (!(c = *(filename_template++)))
                break;
            switch (c) {
            case 'p':
...
int opt_timelimit(void *optctx, const char *opt, const char *arg)
{
    ...
    struct rlimit rl = { static_cast<rlim_t>(lim), static_cast<rlim_t>(lim + 1) };
...
{
    const AVCodecDescriptor * const *da = static_cast<const AVCodecDescriptor *const *>(a);
    const AVCodecDescriptor * const *db = static_cast<const AVCodecDescriptor *const *>(b);
...
void * grow_array(void *array, int elem_size, int *size, int new_size)
{
    ...
    if (*size < new_size) {
        ...
        *size = new_size;
        return reinterpret_cast<Option *>(tmp);
    }
    return static_cast<Option *>(array);
}
